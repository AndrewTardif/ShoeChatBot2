...
/**
 * 
 * A HttpGet class to send GET requests to a URL resource.
 * 
 * @author Raghav
 *
 */
public class HttpGet {
	 ...
	 * @param storeCookies Whether to store cookies or not
	 ...
	 * @param url The URL to request
	 * @param storeCookies Whether to store cookies or not
	 * @param proxyIp The proxy IP to use
	...
	/**
	 * @return A response string that contains the html content of the requested page.
	 */
	public String get() {
	...
	/**
	 * @return A byte array that represents the content on the requested page.
	 */
	public byte[] readAndGetBytes() {
		URL obj;
		...
		if (con != null) {
			...
			try {
				...
				setHeaders(con);
				//Create byte[] array.
				byte[] responseBytes = new byte[Integer.parseInt(con.getHeaderField("content-length"))];
				// Read bytes
				try(
				...
				System.out.println("No header content-length set.");
			}
		}
		// Return null. If there was valid content, it would have been returned in
		// return html.toString();
	 ...
	 * 
	 */
	private String readAndGetResponse(HttpURLConnection con, boolean errorStream) throws IOException {
